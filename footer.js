...
if(!getCookie('justpetrov_cookie_consent')){const box=document.createElement('section');box.className='cookie-consent';box.setAttribute('role','dialog');box.setAttribute('aria-label','Cookie consent');box.innerHTML='<h2>We Love Cookies</h2><p>We use cookies to remember your cookie preferences and keep the website working properly. Choose below whether you want to accept or decline cookies.</p><div class="cookie-actions"><button class="cookie-accept" type="button">Accept</button><button class="cookie-reject" type="button">Decline</button></div>';document.body.appendChild(box);const finish=value=>{setCookie('justpetrov_cookie_consent',value);box.remove();window.dispatchEvent(new CustomEvent('justpetrov-cookie-consent',{detail:{consent:value}}))};box.querySelector('.cookie-accept').onclick=()=>finish('accepted');box.querySelector('.cookie-reject').onclick=()=>finish('rejected')}
const addAdminDashboardLink=()=>{document.querySelectorAll('footer .contact').forEach(contact=>{if(contact.querySelector('.admin-dashboard-link'))return;const a=document.createElement('a');a.className='admin-dashboard-link';a.href='/admin/';a.textContent='Admin Dashboard';contact.appendChild(a)})};
if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',addAdminDashboardLink);else addAdminDashboardLink();
})();
